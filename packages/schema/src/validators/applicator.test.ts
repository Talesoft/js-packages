...
describe('applicatorValidators', () => {
  describe('prefixItems', () => {
    it('should produce correct verbose output for valid state', () => {
      return validateVerbose({ $id: 'test', prefixItems: [{ type: 'number' }] }, [2]).then(result =>
        expect(result).toEqual({
          ...
          annotations: [
            {
              absoluteKeywordLocation: 'test#/prefixItems',
              valid: true,
              keywordLocation: '#/prefixItems',
              instanceLocation: '#',
              annotations: [
                {
                  absoluteKeywordLocation: 'test#/prefixItems/0',
                  valid: true,
                  keywordLocation: '#/prefixItems/0',
                  instanceLocation: '#/0',
                  annotations: [
                    {
                      absoluteKeywordLocation: 'test#/prefixItems/0/type',
                      valid: true,
    ...
    it('should produce correct verbose output for invalid state', () => {
      return validateVerbose({ $id: 'test', prefixItems: [{ type: 'number' }] }, ['test']).then(
        result =>
          expect(result).toEqual({
            absoluteKeywordLocation: 'test#',
            valid: false,
    ...
    it('should ignore values out of bounds', () => {
      return validateFlag(
        { prefixItems: [{ type: 'string' }, { type: 'number' }, { type: 'string' }] },
        ['test 1'],
      ).then(result => expect(result).toEqual({ valid: true }))
    })

    it('should ignore values not defined', () => {
      return validateFlag({ prefixItems: [{ type: 'string' }] }, ['test 1', 12, null, [], {}]).then(
        result => expect(result).toEqual({ valid: true }),
      )
    })
  })
  ...
  describe('items', () => {
    it('should produce correct verbose output for invalid state', () => {
      return validateVerbose({ items: { type: ['string', 'number'], minLength: 3, maximum: 10 } }, [
        3,
        'abc',
        'ab',
        12,
  ...
  describe('additionalProperties', () => {
    each([
      ...
      return validateFlag(schema, value).then(result => expect(result).toEqual(expected))
    })
  })
})
