...
export type ColorConverterMap = {
  [FromSpace in ColorSpace]: { [ToSpace in ColorSpace]: ColorConverter }
}
...
export const colorConverters: ColorConverterMap = {
  ...
  [ColorSpace.RGB]: {
    ...
    [ColorSpace.HSLA]: color =>
      new Color(ColorSpace.HSLA, [...toSpace(color, ColorSpace.HSL).data, 1]),
  },
  // RGBA
  [ColorSpace.RGBA]: {
    // RGBA -> RGB
    ...
    // RGBA -> HSLA
    [ColorSpace.HSLA]: color =>
      new Color(ColorSpace.HSLA, [...toSpace(color, ColorSpace.HSL).data, color.data[3]]),
  },
  [ColorSpace.HSL]: {
    // HSL -> RGB
    ...
    [ColorSpace.RGBA]: color =>
      new Color(ColorSpace.RGBA, [...toSpace(color, ColorSpace.RGB).data, 1]),
    // HSL -> HSL
  ...
  [ColorSpace.HSLA]: {
    ...
    // HSLA -> RGBA
    [ColorSpace.RGBA]: color =>
      new Color(ColorSpace.RGBA, [...toSpace(color, ColorSpace.RGB).data, color.data[3]]),
    // HSLA -> HSL
...
export const toSpace = (color: Color, targetSpace: ColorSpace): Color => {
...
const getRgbFromHue = (p: number, q: number, t: number): number => {
  let nt = t
  // Normalize
  if (nt < 0) {
    nt += 1
